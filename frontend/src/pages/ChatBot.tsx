...
const ChatBot: React.FC<ChatBotProps> = () => {
  ...
  const handleSelectConversation = useCallback(async (id: string) => {
    ...
    try {
      ...
      setErrorDialog({ open: true, message: "Error al cargar los mensajes de la conversación." });
      setMessages([]);
    }
  ...
  const handleRenameConversation = useCallback(async (id: string, newTitle: string) => {
    ...
    try {
      ...
      if (!success) {
        ...
        setErrorDialog({ open: true, message: "Error al renombrar la conversación. Por favor, inténtalo de nuevo." });
      ...
      setErrorDialog({ open: true, message: "Error de conexión al renombrar la conversación. Por favor, inténtalo de nuevo." });
      const originalConv = conversations.find((c) => c.id === id);
      if (originalConv) {
        setConversations(prev => prev.map((conv) => (conv.id === id ? originalConv : conv)));
        if (currentChat?.id === id) {
  ...
  const confirmDeleteConversation = useCallback(async () => {
    ...
    try {
      ...
      if (!success) {
        ...
        if (convToDelete) {
          setConversations(prev => [...prev, convToDelete]);
        }
      } else {
  ...
  }, [conversationToDelete, conversations, activeConversationId, handleNewConversation]);

  const handleQuickAction = useCallback((action: QuickAction, originalMessage: Message) => {
    console.log("⚡ Acción rápida:", action.id);
    console.log("📝 Mensaje original:", originalMessage.id, originalMessage.content.substring(0, 100));
    ...
    if (action.id === "mapa-mental") {
      ...
      if (convId) {
        ...
        generarMapaMental(originalMessage.content, convId);
      } else {
        setErrorDialog({ open: true, message: "Debes tener una conversación guardada para generar un mapa mental." });
      }
    } else {
      ...
      switch (action.id) {
        case "resumen":
          ...
          break;
        case "explicar":
          actionText = "Por favor, explica mejor el mensaje que estoy citando";
          break;
        case "ejemplo":
          actionText = "Por favor, dame ejemplos relacionados con el mensaje que estoy citando";
          break;
        default:
