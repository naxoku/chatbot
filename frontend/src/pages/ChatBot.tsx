...
const DocumentsModal = React.lazy(() => import("@/components/DocumentsModal").then(module => ({ default: module.DocumentsModal })));
const ArtifactsModal = React.lazy(() => import("@/components/ArtifactsModal").then(module => ({ default: module.ArtifactsModal })));
const MindMapModal = React.lazy(() => import("@/components/MindMapModal").then(module => ({ default: module.MindMapModal })));
...
const ChatBot: React.FC<ChatBotProps> = () => {
  ...
  const { setIsAuthenticated, isAuthenticated } = React.useContext(AppContext);

  React.useEffect(() => {
    const title = conversationId 
      ? `Chatbot - Conversación ${conversationId}` 
      : "Chatbot - Asistente virtual VRAE";
  ...
  const handleNewConversation = useCallback(() => {
    ...
    // Navegar a /chat sin ID para nueva conversación
    navigate('/chat');
  ...
  const handleSelectConversation = useCallback(async (id: string) => {
    ...
    // Navegar a la URL con el ID de la conversación
    navigate(`/chat/${id}`);
  ...
  }, [conversations, loadConversationMessages, navigate]);
  ...
  const handleDeleteConversation = useCallback(async (id: string) => {
    ...
    if (activeConversationId === id) {
      ...
      if (remaining.length > 0) {
        // Si quedan conversaciones, seleccionar la primera
        handleSelectConversation(remaining[0].id);
      } else {
        // Si no quedan conversaciones, ir a nueva conversación
  ...
  useEffect(() => {
    ...
    // Solo cargar conversaciones si el usuario está autenticado
  ...
  // Efecto para cargar conversación desde URL
  useEffect(() => {
    if (conversationId && conversations.length > 0) {
      console.log("🔗 Cargando conversación desde URL:", conversationId);
      const conv = conversations.find((c) => c.id === conversationId);
      if (conv && activeConversationId !== conversationId) {
        // Solo cargar si no es la conversación ya activa
        handleSelectConversation(conversationId);
      } else if (!conv) {
        console.warn("⚠️ Conversación no encontrada:", conversationId);
        // Si la conversación no existe, redirigir a /chat
        navigate('/chat', { replace: true });
      }
    } else if (!conversationId && conversations.length > 0 && !currentChat) {
      // Si estamos en /chat sin ID y no hay conversación activa, mostrar estado vacío
      console.log("📭 Estado vacío: sin conversación activa");
      setMessages([]);
      setQuotedMessage(null);
    }
  }, [conversationId, conversations, activeConversationId, currentChat, handleSelectConversation, navigate]);
    ...
    <ThemeProvider defaultTheme="system" storageKey="vite-ui-theme">
      <div className="h-screen bg-background flex overflow-hidden">
        ...
        <main id="main-content" className="flex-1 flex flex-col min-w-0 h-full">
          ...
          <div className="flex-1 overflow-y-auto p-6" aria-live="polite" aria-atomic="false">
            {messages.length === 0 ? (
              <EmptyChatState
                conversationTitle={currentChat?.title || "Nueva Conversación"}
              />
              ...
              <div className="space-y-4 max-w-4xl mx-auto">
                <h2 className="sr-only">Conversación actual</h2>
